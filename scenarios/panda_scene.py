...
# from pydrake.multibody.inverse_kinematics import DifferentialInverseKinematicsIntegrator, DifferentialInverseKinematicsParameters
...
from pydrake.all import (
    ...
    DiagramBuilder, FindResourceOrThrow, GeometryInstance, LeafSystem_,
    ...
    Sphere, StateInterpolatorWithDiscreteDerivative, UnitInertia, Simulator,
...
def panda_traj_controller(traj_pos, traj_rot, traj_hand, panda_plant):
    ...
    b.Connect(traj_to_rigid.get_output_port(), ik.GetInputPort("X_WE_desired"))
...
plant.WeldFrames(
    ...
    frame_on_child_M=plant.GetFrameByName("panda_link0", panda),
    X_FM=RigidTransform(np.array([0.0, 0.5, 0.0]))
)
...
controller_plant.WeldFrames(
    ...
    frame_on_child_M=controller_plant.GetFrameByName("panda_link0",
                                                     control_only_panda),
    X_FM=RigidTransform(np.array([0.0, 0.5, 0.0]))
)
